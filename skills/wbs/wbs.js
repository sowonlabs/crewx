...
function initDB() {
  ...
  alasql('CREATE TABLE IF NOT EXISTS jobs (id STRING PRIMARY KEY, wbs_id STRING, title STRING, description STRING, agent STRING, status STRING, seq INT, created_at STRING, completed_at STRING)');
      ...
      if (data.jobs && data.jobs.length > 0) {
        data.jobs.forEach(j => {
          alasql('INSERT INTO jobs VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)',
            [j.id, j.wbs_id, j.title, j.description || null, j.agent, j.status, j.seq, j.created_at, j.completed_at]);
        });
      }
...
const wbs = {
   ...
   */
  addJob(wbsId, { title, description = null, agent, seq = 0 }) {
    const id = `job-${Date.now()}-${seq}`;
    const now = new Date().toISOString();
    alasql('INSERT INTO jobs VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)',
      [id, wbsId, title, description, agent, 'pending', seq, now, null]);
    saveData();
    return id;
  },
...
function handleJobCommand(args) {
  ...
  switch (subCommand) {
    ...
    case 'add': {
      ...
      const { title, agent, desc, description } = options;
      const seq = options.seq ? parseInt(options.seq, 10) : 0;
      const jobDesc = desc || description || null;

      if (!wbsId || !title || !agent) {
        console.error('Usage: wbs.js job add <wbs-id> --title "Job Title" --agent "@crewx_codex_dev" [--desc "상세 지시"] [--seq N]');
        process.exit(1);
      }
      const jobId = wbs.addJob(wbsId, { title, description: jobDesc, agent, seq });
      console.log(JSON.stringify({ jobId, wbsId, title, description: jobDesc, agent, seq }, null, 2));
      break;
    }
    ...
    case 'update': {
      const jobId = args[1];
      const options = parseOptions(args, 2);
      const { status, title, agent, seq, desc, description } = options;
      const jobDesc = desc || description;

      if (!jobId) {
        console.error('Usage: wbs.js job update <job-id> [--status <status>] [--title "..."] [--desc "..."] [--agent "@..."] [--seq N]');
      ...
      if (updates.length === 0) {
        console.error('No update options provided. Use --status, --title, --desc, --agent, or --seq');
        process.exit(1);
      }
...
Examples:
  wbs.js job list wbs-123
  wbs.js job list wbs-123 --json
  wbs.js job add wbs-123 --title "Design schema" --agent "@claude:sonnet" --seq 1
  wbs.js job update job-456 --status completed
  wbs.js job retry job-456
  wbs.js job next wbs-123
...
if (require.main === module) {
  ...
  switch (command) {
    ...
    case 'create': {
      ...
      // Generate ID first for detail file naming
      const id = `wbs-${Date.now()}`;
